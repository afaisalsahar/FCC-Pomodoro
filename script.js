...
var pomodoroTimer = new (function() {
    var countdown, mode = 0, isRunning = false,
		timeFocus = 150000, timeRest = 30000, timeIncrements = 1000, timeNow = 0,
		...
		updateStatus = function(status) {
			$("#status").html((status) ? status : (mode == 0) ? "Focus" : "Rest");
		}

		switchTimer = function() {
			if(!mode) {
				mode = 1; timeNow = timeRest;
			} else {
		...
		updateTimerSettings = function() {
			...
			timeRest = Number($("#restLength").val()) * 6000;
		...
		audioNotification = function() {
			if(!audioLoad) {
				audio = $("<audio>").attr("src", "sound-notification.mp3");
				audioLoad = true;
			}
